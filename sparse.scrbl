...
@title{Sparse: Test Generation for Simple S-expression Language Parsers}

 Sparse is a tool (written in Racket) for
 ...
 implemented and produces test cases for the students'
 parsers.

@section{Motivation}

Producing test cases for parsers by hand is tedious and
...
must be updated--and doing so may not always be a simple
find-and-replace. It would be nice to have a tool which
automatically generates exhaustive test cases based solely
on the language's grammar. This is exactly what Sparse does.

@subsection{Example}

Consider the following grammar for a simple language (dubbed
...
Here, @racket[Number], @racket[String], and @racket[Id] are
not to be taken literally, but rather as placeholders for
literal numbers, strings, and identifiers. Additionally,
@racket[if], @racket[var], @racket[lam], and @racket[=] are to be forbidden as
identifiers. Built-in operations, functions, and constants
like @racket[+], @racket[*], @racket[<=], @racket[substring], or @racket[true] are treated like
...
@codeblock|{{{lam {x y} {if {<= x y} x y}} 5 4}}|

which introduces a function to determine the minimum of two
...
students' parsers would be tedious--and would likely fail to
be comprehensive.

Sparse takes as input a description of the grammar which is
not unlike the grammar shown above. It is worth noting here
the largest limitation of Sparse. Sparse's input grammar may
only have one nonterminal. In this example, this is @racket[Expr].
...
The details of the format of the input grammar are described
below. Using this grammar, one can call
@racket[generate-valid-testcase] to generate one large test case
which will test to make sure the student's parser accepts
...
Additionally, one can call @racket[generate-invalid-testcases] to
generate a list of test cases which do *not* conform to the
grammar to make sure that the student's parser rejects all
...
{if 0 "Hello" eq? "World"}
{if <= 1 -1 substring ""}
{if "Hello" array 2.2 new-array "World" -22/7}
{= aref}
{"" =}
{lam {true = e} f}
{var {lam = ""} "World"}
...
{var {eq? = g "World"} l}
{var {<= = 2.2 h ""} "World"}
{var {substring = -22/7 i "Hello" 0} -22/7}
{var {array = m} if}
{var}
{var {new-array = ""}}
{var {begin = "World"} o ""}
{var {a = -1} 2.2 p "Hello"}
{var {b = q} -22/7 r "World" 0}
{= {true} s}
{lam {lam} ""}
{lam {aref} =}
{lam}
...
Since the "valid" test case produced is so large, it would
...
minimal failing test cases for a student's parser. For
example, for one student's parser, the following minimal
failing test cases were identified:

@codeblock|{
{lam {true false null +} false}
{lam {- * /} "Hello"}
{lam {eq? <=} 1}
{lam {array} 0}
{var {false = "World"} x}
...
Here, the student's mistake was forbidding the built-ins as
parameter names; the language's grammar and semantics allow
redefining the names. It is worth noting that this student
had actually passed all of the instructor's original test
cases. Since the instructor's test cases only tried
...
cases and make the test suite more comprehensive.
...
@defproc[(generate-valid-testcase [grammar grammar?]) s-expression?]{
 Using the grammar, produces one large S-expression which
 ...
 have a subexpression like @code|{{if _ {var {_ = _} _} _}}|. A
 @racket[var] expression is guaranteed to appear in each of the
 three subexpressions of an @racket[if] expression. Likewise, each
 different 'type' of expression is guaranteed to appear in
 each spot where a subexpression is allowed. This process is
 deterministic; there's no point in generating multiple
 expressions from the same grammar.}

@defproc[(generate-invalid-testcases [grammar grammar?]) (listof s-expression?)]{
 Using the grammar,
 ...
 grammar will produce expressions that use @racket[lam] in places
 where @racket[lam] is not allowed, and will create @racket[if] expressions
 that don't have exactly three subexpressions.}

@defproc[(generate-valid-testcase-for-minimization [grammar grammar?]) s-expression?]{
 Produces an expression just like @racket[generate-valid-testcase],
...
@defproc[(produce-minimum-failing-testcases
         ...
         (listof s-expression?)]{
Given a test case
 generated by @racket[generate-valid-testcase-for-minimization] and
 a student's parse function, produces a list of "minimal"
 test cases on which the student's parser fails (i.e., throws
 an exception). The list of test cases is not exhaustive in
 the sense that there might be expressions on which the
 parser fails that won't be in the list until other
 expressions in the list have successfully parsed. The list
 of expressions will only be empty if the student's parser
 ...
 function. The @racket[preprocess] argument is applied to any
 expression before it is passed to the student's parser. This
 is useful if, for instance, the language in question has a
 top-level construct around which expressions must be
 wrapped.}

@defproc[(strip-minimization-info [minimization-testcase s-expression?]) s-expression?]{
 Strips
 the annotations from the return values of
 @racket[generate-valid-testcase-for-minimization] to give a pure
 S-expression. @racket[(strip-minimization-info
 (generate-valid-testcase-for-minimization grammar))] will
 produce the same value as @racket[(generate-valid-testcase
 grammar)].}
